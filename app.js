...
app.post("/register", async (req, res) => {
  ...
  if (phonenumber.length !=10) {
    return res.status(400).send("Phone number must be length of 10");
  }

  //check length of voterid
  ...
  try {
    ...
    // Define a unique folder for this user's face images (for example, using their email).
    // Replace '@' and '.' with underscores so the folder name is filesystem friendly.
    const faceFolder = `captured_faces/${mailid.replace(/[@.]/g, "_")}`;

    // **Face Capture Step:**  
    // Run the Python face capture script synchronously so that the registration process waits for it.
    // Adjust "python" to "python3" if necessary.
    const captureResult = spawnSync("python", ["face_capture.py", faceFolder], { encoding: "utf8" });
    ...
    // Insert the user details along with the face folder path into the database.
    // Ensure that your "users" table has a "face_folder" column (of type TEXT).
    await pool.query(
...
app.post("/give-vote", async (req, res) => {
  try {
    ...
    if (output.startsWith("Face recognized:")) {
      const userId = output.split(":")[1].trim();
      res.sendFile(__dirname + "/vote.html");
    } else {
      res.status(401).send("Face not recognized. Access denied. <a href='/'>Go to Home</a>");
    }
  } catch (err) {
    console.error(err);
